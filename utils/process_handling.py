...
class batch_process(widgets.GridBox):
    ...
    def __init__(
        ...
        steps = []
        plan_position_offset = process_list[0][
            'positon_in_experimental_plan'
        ]  # the number indicating the first manufacturing step, might be 0 or 1
        ...
        def create_step_description(i):
            if len(self.steps[i]) == 1:
                label = widgets.Label(
                    value=f'{int(self.steps[i][0]["positon_in_experimental_plan"])} {self.steps[i][0]["name"]}'
                )
            elif len(self.steps_merged[i]) == 1:
                label = widgets.Label(
                    value=f'{int(self.steps[i][0]["positon_in_experimental_plan"])} {self.steps[i][0]["name"]}, {len(self.steps[i])} variations',
                    style={'text_color': 'blue'},
                )
            else:
                label = widgets.Label(
                    value=f'{int(self.steps[i][0]["positon_in_experimental_plan"])} {len(self.steps_merged[i])} different step types',
        ...
        super().__init__(
            list(itertools.chain.from_iterable(map(create_step_description, range(len(self.steps))))),
            layout=widgets.Layout(grid_template_columns='3fr 1fr', border='1px solid black'),
        )
        ...
        def param_selection_buttons(step_index):
            ...
            return list(
                map(
                    lambda process: list(
                        map(
                            lambda jsonpath_values_tuple: manufacturing_parameter(
                                jsonpath_values_tuple[0],
    ...
    def show_process_details(self, step_index):
        ...
        with self.detail_widget:
            for process_idx, process in enumerate(self.steps_merged[step_index]):
                ...
                display(
                    widgets.GridBox(
                        self.manufacturing_params[step_index][process_idx],
    ...
    def samples_for_processes_in_step(self, step_index):
        return list(
            map(
                lambda process: list(
                    map(lambda sample: sample.get('lab_id', 'id not found'), process['samples'])
                ),
        ...
        with self.parameter_widget:
            ...
            display(
                widgets.GridBox(
                    list(
                        filter(
                            lambda button: button.value,
                            itertools.chain.from_iterable(
                                itertools.chain.from_iterable(self.manufacturing_params)
                            ),
                        )
                    ),
    ...
    def get_selected_manufacturing_parameters(
        self, only_samples_with_efficiencies=True, only_samples_with_all_values=True
    ):
        datapoints_dict = {}
        parameter_name_list = []
        for parameter_object in filter(
            lambda x: x.value,
            ...
            for sample_id in filter(
                lambda x: bool(x in self.efficiencies or not only_samples_with_efficiencies),
                parameter_object.samples_values_dict,
            ):
                if sample_id not in datapoints_dict:
                    datapoints_dict[sample_id] = {}
                datapoints_dict[sample_id][str(parameter_object.keychain[-1])] = (
                    parameter_object.samples_values_dict[sample_id]
                )
...
class manufacturing_parameter(widgets.ToggleButton):
    ...
    def __init__(self, keychain, sample_ids_nested, values, update_callback):
        ...
        self.value = False
        self.samples_values_dict = dict(
            itertools.chain.from_iterable(
                map(
                    lambda idx: map(lambda sample_id: (sample_id, values[idx]), sample_ids_nested[idx]),
...
def merge_step_data(initial_process_list, exclude_non_numeric=False, blacklist=[]):
    ...
    process_list = [
        {key: unfiltered_process[key] for key in set(unfiltered_process) - set(ignored_keys)}
        for unfiltered_process in initial_process_list
    ]
...
def merge_process(process_list, exclude_non_numeric, recursion_depth):
    ...
    assert all(
        map(lambda x: bool(type(x) == type(process_list[0])), process_list)
    )  # raise assertion error if not all processes have the same datatype
    if isinstance(process_list[0], dict) or isinstance(process_list[0], list):
        ...
        for keyset_index, keyset in enumerate(key_structures):
            ...
            key_results = dict(
                filter(
                    lambda keyresulttuple: all(
                        map(lambda data: len(data['json']), keyresulttuple[1])
                    ),  # drop keys for which there is no data
                    map(
                        lambda key: (
                            key,
                            merge_process(
                                list(
                                    map(lambda x: x[key], np.array(process_list)[process_indices])
                                ),  # operate on the entries behind KEY for every process
                                exclude_non_numeric,
                                recursion_depth + 1,
                            ),  # additional arguments for merge_process()
                        ),
                        keyset,
                    ),
                )
            ...
            substructure_map = np.array(
                list(
                    map(
                        lambda key: list(
                            map(
                                lambda i: np.argmax(list(map(lambda x: i in x['indices'], key_results[key]))),
            ...
            structure_combinations, process_to_combination = np.unique(
                substructure_map, axis=1, return_inverse=True
            )
            if is_list:
                for unique_structure_index, unique_structure in enumerate(structure_combinations.T):
                    indices_for_current_struc = np.nonzero(process_to_combination == unique_structure_index)[
                        0
                    ]
                    return_list.append(
                        {
                            'indices': process_indices[indices_for_current_struc],
                            'json': list(
                                map(
                                    lambda key: key_results[key][
                                        substructure_map[key][indices_for_current_struc[0]]
                                    ]['json'],
                                    key_results,
                                )
                            ),
                    ...
                    return_list.append(
                        {
                            ...
                            'json': dict(
                                map(
                                    lambda i: (
                                        list(key_results)[i],
                                        key_results[list(key_results)[i]][
                                            substructure_map[i][indices_for_current_struc[0]]
                                        ]['json'],
                                    ),
    ...
    elif isinstance(process_list[0], str) and exclude_non_numeric:
...
def make_table(process, index_map, remove_constants=False, remove_string=False, abbreviate_keys=False):
    ...
    if abbreviate_keys:
        return pd.DataFrame(
            dict(map(lambda x: (str(x[0][-1]), x[1]), column_data)),
            index=list(map(lambda idx: str(index_map[idx]), process['indices'])),
        )
    else:
        return pd.DataFrame(
            dict(map(lambda x: (str(x[0]), x[1]), column_data)),
            index=list(map(lambda idx: str(index_map[idx]), process['indices'])),
        )
...
def flatten_layers(data, keychain, remove_constants, remove_string):
    if isinstance(data, np.ndarray):
        ...
        return list(
            itertools.chain.from_iterable(
                map(
                    lambda idx_entry: flatten_layers(
                        idx_entry[1], keychain + [str(idx_entry[0])], remove_constants, remove_string
                    ),
                    enumerate(data),
                )
            )
        ...
        return list(
            itertools.chain.from_iterable(
                map(
                    lambda key: flatten_layers(data[key], keychain + [key], remove_constants, remove_string),
                    data,
                )
            )
        )
