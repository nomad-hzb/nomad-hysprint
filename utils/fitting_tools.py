...
def stretched_exponential_params(power, times):
    ...
    initial_params = stretched_exponential_model.make_params(
        A=power[0], tau=times[-1], beta=1
...
def biexponential_params(power, times):
    ...
    initial_params = biexp_model.make_params(
        exp1_amplitude=power[0], exp2_amplitude=power[0] / 10, exp1_decay=times[-1], exp2_decay=times[-1] / 10
    )
...
def logistic_params(power, times):
    ...
    initial_params = log_exp_model.make_params(
        A=power[0] / 2, tau=times[-1], L=power[0] / 2, k=times[-1] / 5, x0=times[-1] / 2
    )
...
def erfc_params(power, times):
    ...
    initial_params = erfc_model.make_params(PCE0=power[0], k=times[-1] / 10, t0=times[-1] / 2, b=0)
