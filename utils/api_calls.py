...
def init_cache():
    ...
    requests_cache.install_cache(
        'my_local_cache', allowable_methods=('GET', 'POST'), ignored_parameters=['Authorization']
    )
...
def get_sample_description(url, token, sample_ids):
    ...
    response = requests.post(f'{url}/entries/query', headers={'Authorization': f'Bearer {token}'}, json=query)
    response.raise_for_status()
...
def get_specific_data_of_sample(url, token, sample_id, entry_type, with_meta=False):
    ...
    for ldata in linked_data:
        if (
            ...
            or entry_type not in ldata['archive']['metadata']['entry_type']
        ):
            continue
        if with_meta:
...
def get_all_JV(url, token, sample_ids, jv_type='HySprint_JVmeasurement'):
    ...
    query = {
        ...
        'owner': 'visible',
        'query': {'entry_references.target_entry_id:any': entry_ids, 'entry_type': jv_type},
        'pagination': {'page_size': 10000},
    }
    response = requests.post(
        f'{url}/entries/archive/query', headers={'Authorization': f'Bearer {token}'}, json=query
...
def get_all_eqe(url, token, sample_ids, eqe_type='HySprint_EQEmeasurement'):
    ...
    query = {
        ...
        'owner': 'visible',
        'query': {'entry_references.target_entry_id:any': entry_ids, 'entry_type': eqe_type},
        'pagination': {'page_size': 100},
    }
    response = requests.post(
        f'{url}/entries/archive/query', headers={'Authorization': f'Bearer {token}'}, json=query
...
def get_all_mppt(url, token, sample_ids, mppt_type='HySprint_SimpleMPPTracking'):
    ...
    query = {
        ...
        'owner': 'visible',
        'query': {'entry_references.target_entry_id:any': entry_ids, 'entry_type': mppt_type},
        'pagination': {'page_size': 100},
    }
    response = requests.post(
        f'{url}/entries/archive/query', headers={'Authorization': f'Bearer {token}'}, json=query
...
def get_efficiencies(url, token, sample_ids):
    query = {
        'required': {
            'results': {
                'properties': {'optoelectronic': {'solar_cell': {'efficiency': '*'}}},
                'eln': {'lab_ids': '*'},
            }
        },
        'query': {
    ...
    return dict(
        map(
            lambda x: (
                x['archive']['results']['eln']['lab_ids'][0],
