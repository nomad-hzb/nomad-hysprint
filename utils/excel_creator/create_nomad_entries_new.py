...
def map_solutions(data):
    ...
    for solvent in sorted(set(solvents)):
        ...
        archive["solvent"].append({"chemical_2": {"name": get_value(data, f"{solvent} name", None, False),
                                                  "load_data": False
                                                  },
    ...
    for solute in sorted(set(solutes)):
        if not get_value(data, f"{solute} type", None, False) and not get_value(data, f"{solute} Concentration [mM]", None):
            continue
        archive["solute"].append({"chemical_2": {"name": get_value(data, f"{solute} type", None, False),
                                                 "load_data": False
                                                 },
