...
def map_spin_coating(i, j, lab_ids, data):
    ...
    archive = {"data": {
        ...
        "datetime": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
        "layer": [{"layer_type": get_value(data, "Layer type", None, False),
                  "layer_material_name": get_value(data, "Material name", None, False)
                   }],
        "solution": [
            {
                "solution_details": {"solvent": [{"chemical_2": {"name": get_value(data, "Solvent", None, False),
                                                                 "load_data": False
                                                                 }}],
                                     "solute": [{"chemical_2": {"name": get_value(data, "Material name", None, False),
                                                                "load_data": False
                                                                },
                                                 "concentration_mol": convert_quantity(get_value(data, "Concentration [mM]", None), 1/1000)}]},  # check unit
                # check unit
...
def map_sdc(i, j, lab_ids, data):
    ...
    archive = {"data": {
        ...
        "quenching": {
            ...
            "air_knife_distance_to_thin_film": convert_quantity(data.get("Air knife gap [cm]", None), 10000),
        }
    }}
...
def map_substrate(data):
    archive = {"data": {
        ...
        "name": "Substrate " + get_value(data, "Sample dimension", "", False) + " " + get_value(data, "Substrate material", "", False) + " " + get_value(data, "Substrate conductive layer", "", False),
        "datetime": datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"),
...
def map_evaporation(i, j, lab_ids, data):
    ...
    if get_value(data, "Organic", "", False).lower().startswith("n"):
        archive["data"].update({"inorganic_evaporation": [{
    ...
    if get_value(data, "Organic", "", False).lower().startswith("y"):
        archive["data"].update({"organic_evaporation": [{
            "thickness": get_value(data, "Thickness [nm]"),
            "start_rate": get_value(data, "Rate [angstrom/s]"),
            "temperature": [get_value(data, "Temperature [°C]", None)]*2 if get_value(data, "Temperature [°C]", None) else None,
            "chemical_2": {
                "name": get_value(data, "Material name", None, False),
                "load_data": False
            }
...
for i, sub in df["Experiment Info"][["Sample dimension", "Sample area [cm^2]", "Substrate material", "Substrate conductive layer"]].drop_duplicates().iterrows():
...
for i, row in df["Experiment Info"].iterrows():
    ...
    substrate_name = find_substrate(
        row[["Sample dimension", "Sample area [cm^2]",
             "Substrate material", "Substrate conductive layer"]]) + ".archive.json"
