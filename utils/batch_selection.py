...
def create_batch_selection(url, token, load_data_function):
    ...
    for b in batch_ids_list_tmp:
        if (
            '_'.join(b.split('_')[:-1]) in batch_ids_list_tmp
        ):  # skip id if the same name without the last word is also in the list
            continue
        batch_ids_list.append(b)
    batch_ids_selector = widgets.SelectMultiple(
        options=batch_ids_list, description='Batches', layout=widgets.Layout(width='400px', height='300px')
    )
