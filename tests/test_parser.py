...
def test_hy_batch_parser_new_cols(monkeypatch):  # noqa: PLR0915
    ...
    for m in measurement_archives:
        ...
        # Step 4: Dip Coating
            ...
            assert m.data.solution[0].solution_details.solute[2].concentration_molar == 1.42 * ureg(
                'mM')
            assert m.data.solution[0].solution_details.solute[3].chemical_2.name == 'Lead Iodide'
            assert m.data.solution[0].solution_details.solute[3].concentration_molar == 1.42 * ureg(
                'mM')
            assert m.data.solution[0].solution_details.solute[4].chemical_2.name == 'Lead Iodide'
            assert m.data.solution[0].solution_details.solute[4].concentration_molar == 1.42 * ureg(
                'mM')
        ...
        # Step 5: Slot Die Coating
            ...
            assert m.data.solution[0].solution_details.solute[1].concentration_molar == 1.42 * ureg(
                'mM')
            assert m.data.properties.flow_rate == 25 * ureg('ul/minute')
            ...
            # assert m.data.drying_speed == 30 * ureg('cm/minute') ##### needs to be added to schema
            assert m.data.annealing.time == 30 * ureg('minute')
            assert m.data.annealing.temperature == ureg.Quantity(120, ureg('°C'))
            assert m.data.annealing.atmosphere == 'Nitrogen'
            assert m.data.description == 'Test annealing'
        ...
        # Step 7: Evaporation
            ...
            assert m.data.organic_evaporation[0].temperature[0] == ureg.Quantity(
                150, ureg('°C'))
            assert m.data.organic_evaporation[0].temperature[1] == ureg.Quantity(
                160, ureg('°C'))
            assert m.data.organic_evaporation[0].substrate_temperature == 25 * \
                ureg('°C')
