...
def test_hy_jv_parser(monkeypatch):
    ...
    assert abs(
        archive.data.jv_curve[2].efficiency - 0.37030243333333296) < 1e-6
    delete_json()
...
def test_hy_batch_parser(monkeypatch):
    ...
    for m in measurement_archives:
        if "HySprint_Sample" in str(type(m.data)) or\
           "HySprint_Batch" in str(type(m.data)):
            count_samples_batches += 1
        if "SlotDieCoating" in str(type(m.data)):
            assert m.data.layer[0].layer_material_name == "Me4PACz" or\
                m.data.layer[0].layer_material_name == "MAFA"
