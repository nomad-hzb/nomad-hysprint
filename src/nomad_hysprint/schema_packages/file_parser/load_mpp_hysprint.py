...
def get_dimensions(columns):
    ...
    number_of_sample = max([get_integer(c.split('_')[2][5:]) for c in columns]) + 1
    number_of_pixels = max([get_integer(c.split('_')[3][5:]) for c in columns]) + 1

    return {
        'box': box,
        'number_of_samples': number_of_sample,
...
def rename_columns(df, box, sample_id, pixel_id):
    ...
    for column in df.columns:
        columns_new.update(
            {
                column: column.replace(f'Anlage_Box{box:02d}_', '')
                .replace(f'Probe({sample_id})_', '')
...
def apply_filter_trigger_code(df, box, trigger_code):
    df_filtered = df.loc[(df[f'Anlage_Box{box:02d}_InTriggerSource'] == trigger_code)]
    df_final = df_filtered.drop(columns=[f'Anlage_Box{box:02d}_InTriggerSource'])
    return df_final
...
def parse_sample(info, sample_id, df):
    ...
    data = {'id': sample_id, 'data': df_sample, 'pixels': []}
