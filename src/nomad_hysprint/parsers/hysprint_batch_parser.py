...
class HySprintExperimentParser(MatchingParser):
    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        for i, row in df['Experiment Info'].iterrows():
            ...
            archives.append(map_basic_sample(
                row, substrate_name, upload_id, HySprint_Sample))
            ...
            for j, row in df_dropped.iterrows():
                ...
                if 'Evaporation' in col:
                    archives.append(map_evaporation(i, j, lab_ids, row,
                                    upload_id, HySprint_Evaporation))
                ...
                if 'Slot Die Coating' in col:
                    archives.append(map_sdc(i, j, lab_ids, row,
                                    upload_id, HySprint_SlotDieCoating))

                if 'Sputtering' in col:
                    archives.append(map_sputtering(i, j, lab_ids, row,
                                    upload_id, HySprint_Sputtering))

                if 'Inkjet Printing' in col:
                    archives.append(
                        map_inkjet_printing(i, j, lab_ids, row,
                                            upload_id, HySprint_Inkjet_Printing)
                    )
